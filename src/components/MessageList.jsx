...
const MessageList = ({ searchTerm = '', filters = {} }) => {
  const { messages, currentRoom, user, typingUsers, markAsRead, addReaction } = useChat();
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      {roomMessages.map((message) => (
        message.isSystem ? (
          ...
          >
            <div
              className={`max-w-xs lg:max-w-md px-4 py-2 rounded-lg ${message.user === user?.username
